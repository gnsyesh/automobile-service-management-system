      ...
      <>
        ...
        >
          ...
          <div className="relative h-48 w-full md:w-56 shrink-0 overflow-hidden rounded-xl bg-slate-100 dark:bg-[#111111]">
            ...
            />
            {product.discount && (
              <span className="absolute top-3 left-3 rtl:left-auto rtl:right-3 rounded-full bg-[#8B3A2E] px-2.5 py-1 text-xs font-bold text-white shadow-md">
                -{product.discount}%
              </span>
            )}
          </div>
          ...
          {/* Details */}
          <div className="flex-1 flex flex-col justify-between w-full text-left rtl:text-right">
            <div>
              ...
              <div>
                <div className="flex items-baseline gap-2">
                  <span className="text-2xl font-extrabold text-slate-900 dark:text-white">
                    {product.price.toLocaleString()} <span className="text-sm font-semibold text-[#D4A017]">{t("card.egp")}</span>
                  </span>
                  {product.oldPrice && (
                    <span className="text-sm text-slate-400 dark:text-gray-500 line-through">
                      {product.oldPrice.toLocaleString()} {t("card.egp")}
                    </span>
                  )}
                </div>
              </div>
    ...
    <>
      ...
      >
        ...
        <div className="relative h-56 w-full overflow-hidden rounded-xl bg-slate-100 dark:bg-[#111111] p-4 flex items-center justify-center">
          {/* Badges */}
          <div className="absolute top-3 left-3 rtl:left-auto rtl:right-3 z-10 flex flex-col gap-1.5">
            {product.discount && (
          ...
          <div className="absolute top-3 right-3 rtl:right-auto rtl:left-3 z-10 flex flex-col gap-2 opacity-90 group-hover:opacity-100 transition-opacity">
            <button
              onClick={() => toggleWishlist(product)}
          ...
          <div className="mt-4 pt-3 border-t border-slate-200 dark:border-[#2D2D2D] flex items-center justify-between gap-2">
            <div>
              <div className="text-lg font-extrabold text-slate-900 dark:text-white leading-none">
                {product.price.toLocaleString()} <span className="text-xs text-[#D4A017] font-semibold">{t("card.egp")}</span>
              </div>
              {product.oldPrice && (
