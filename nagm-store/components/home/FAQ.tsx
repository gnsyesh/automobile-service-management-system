        ...
        <div className="space-y-4">
          {faqs.map((faq, idx) => {
            ...
            return (
              <div
                key={idx}
                className="rounded-2xl border border-[#2D2D2D] bg-[#1B1B1B]/80 overflow-hidden backdrop-blur-xl transition-all"
              >
                <button
                  onClick={() => setOpenIdx(isOpen ? null : idx)}
                  className="w-full flex items-center justify-between p-5 text-left font-bold text-white text-base hover:text-[#D4A017] transition"
                >
                  <span>{faq.q}</span>
                ...
                {isOpen && (
                  <div className="px-5 pb-5 text-sm text-gray-300 leading-relaxed border-t border-[#2D2D2D] pt-3">
                    {faq.a}
                  </div>
                )}
              </div>
