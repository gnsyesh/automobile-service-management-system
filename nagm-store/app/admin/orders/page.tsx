...
export default function AdminOrdersPage() {
  ...
  const handleUpdateStatus = async (
    ...
    try {
      ...
      // The server will independently verify that this user
      // actually has active admin privileges.
      // ---------------------------------------------------------
      ...
      // ---------------------------------------------------------
      // All order-status + inventory logic is handled server-side.
      // ---------------------------------------------------------
      ...
      // ---------------------------------------------------------
      // Update local UI only after the server succeeds.
      // ---------------------------------------------------------
      setOrders((prev) =>
        prev.map((o) => {
          if (o.id !== orderId) {
          ...
          let updatedStockDecremented =
            o.stockDecremented;

          if (result.stockChanged === true) {
            if (newStatus === "Cancelled") {
              updatedStockDecremented = false;
            } else if (
              previousStatus === "Cancelled"
            ) {
              updatedStockDecremented = true;
            }
          }
      ...
      if (
        ...
        let updatedStockDecremented =
          selectedOrder.stockDecremented;
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
        <div className="relative w-full sm:max-w-md">
          ...
          <input
            ...
            placeholder={
              language === "ar"
                ? "ابحث برقم الطلب، البريد، أو الهاتف..."
                ...
                {[...Array(6)].map((_, i) => (
                  ...
                  >
                    <td className="py-4 px-4">
                      <div className="h-4 w-20 bg-slate-200 dark:bg-[#252525] rounded" />
                    </td>

                    <td className="py-4 px-4">
                      <div className="h-4 w-32 bg-slate-200 dark:bg-[#252525] rounded mb-1" />
                      <div className="h-3 w-24 bg-slate-100 dark:bg-[#202020] rounded" />
                    </td>

                    <td className="py-4 px-4">
                      <div className="h-4 w-24 bg-slate-200 dark:bg-[#252525] rounded" />
                    </td>

                    <td className="py-4 px-4">
                      <div className="h-4 w-14 bg-slate-200 dark:bg-[#252525] rounded" />
                    </td>

                    <td className="py-4 px-4">
                      <div className="h-4 w-16 bg-slate-200 dark:bg-[#252525] rounded" />
                    </td>
                    ...
                    <td className="py-4 px-4">
                      <div className="h-6 w-20 bg-slate-200 dark:bg-[#252525] rounded-lg" />
                    </td>

                    <td className="py-4 px-4 text-right rtl:text-left">
                      <div className="h-7 w-16 bg-slate-200 dark:bg-[#252525] rounded-lg ml-auto rtl:ml-0 rtl:mr-auto" />
                    </td>
                  </tr>
                ))}
              </tbody>
            ...
            <table className="w-full text-xs text-left rtl:text-right">
              ...
              <tbody className="divide-y divide-slate-200 dark:divide-[#2D2D2D]">
                {filtered.map((ord) => (
                  <tr
                    key={ord.id}
                    className="hover:bg-slate-50 dark:hover:bg-[#181818] transition"
                  >
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm">
          <div className="w-full max-w-2xl rounded-3xl border border-slate-200 dark:border-[#2D2D2D] bg-white dark:bg-[#151515] p-6 sm:p-8 shadow-2xl space-y-6 max-h-[90vh] overflow-y-auto">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 text-xs">
              <div className="rounded-2xl border border-slate-200 dark:border-[#2D2D2D] p-4 bg-slate-50 dark:bg-[#0E0E0E]">
                <div className="text-slate-500 dark:text-gray-400 uppercase tracking-wider font-bold mb-1">
                  Payment Method
                </div>
            ...
            <div className="space-y-3">
              <h4 className="text-xs font-bold text-slate-600 dark:text-gray-400 uppercase">
                Ordered Parts
              </h4>
              ...
              {selectedOrder.items?.map(
                (item) => (
                  <div
                    key={item.product.id}
                    className="flex items-center justify-between gap-3 text-xs border-b border-slate-200 dark:border-[#2D2D2D] pb-2"
                  >
            ...
            <div className="text-right text-xs space-y-1 border-t border-slate-200 dark:border-[#2D2D2D] pt-3">
              ...
              <div>
                ...
                {selectedOrder.shipping === 0
                  ? "FREE"
                  : `${selectedOrder.shipping} EGP`}
              </div>

              <div>
                VAT (14%):{" "}
                {selectedOrder.vat?.toLocaleString()}{" "}
                EGP
              </div>
