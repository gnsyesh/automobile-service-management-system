  ...
  return (
    <main className="min-h-screen bg-slate-50 text-[#0F172A] dark:bg-[#111111] dark:text-gray-100 flex flex-col pt-24 sm:pt-32 pb-20 transition-colors duration-300">
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 w-full flex-1">
        ...
        {/* Breadcrumbs */}
        <nav className="flex items-center gap-2 text-xs font-semibold text-slate-500 dark:text-gray-400 mb-8 overflow-x-auto pb-1">
          <Link href="/" className="hover:text-[#D4A017] transition">Home</Link>
          <ChevronRight className="h-3 w-3 text-slate-400 dark:text-gray-600 shrink-0" />
          ...
          <ChevronRight className="h-3 w-3 text-slate-400 dark:text-gray-600 shrink-0" />
          <span className="text-slate-900 dark:text-white font-bold truncate max-w-[200px]">{product.name}</span>
        </nav>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 items-start">
          ...
          {/* Left Gallery Column */}
          <div className="lg:col-span-6 space-y-4">
            <div className="relative h-[380px] sm:h-[450px] w-full overflow-hidden rounded-3xl border border-slate-200 dark:border-[#2D2D2D] bg-white dark:bg-[#1B1B1B] p-6 flex items-center justify-center shadow-sm dark:shadow-2xl">
              <Image
              ...
              <button
                onClick={handleShare}
                className="absolute top-4 right-4 p-2.5 rounded-full bg-slate-100/90 dark:bg-[#111111]/80 text-slate-600 dark:text-gray-300 hover:text-white hover:bg-[#8B3A2E] dark:hover:bg-[#8B3A2E] transition shadow-md"
              ...
              <div className="flex items-center gap-3 overflow-x-auto pb-2">
                {product.images.map((img, idx) => (
                  <button
                    ...
                    className={`relative h-20 w-20 shrink-0 rounded-2xl overflow-hidden border-2 bg-white dark:bg-[#1B1B1B] transition-all ${
                      selectedImageIndex === idx
                        ? "border-[#D4A017] shadow-lg scale-95"
                        : "border-slate-200 dark:border-[#2D2D2D] opacity-60 hover:opacity-100"
                    }`}
                  >
          ...
          <div className="lg:col-span-6 space-y-6">
            ...
            <div>
              <div className="flex items-center justify-between gap-2 mb-2">
                ...
                <span className={`text-xs font-extrabold px-3 py-1 rounded-full border ${
                  product.inStock ? "border-emerald-500/30 bg-emerald-500/10 text-emerald-600 dark:text-emerald-400" : "border-red-500/30 bg-red-500/10 text-red-600 dark:text-red-400"
                }`}>
            ...
            {/* Price Banner */}
            <div className="rounded-2xl border border-slate-200 dark:border-[#2D2D2D] bg-white dark:bg-[#1B1B1B]/80 p-5 flex items-baseline gap-4 shadow-sm dark:shadow-lg">
              <span className="text-3xl sm:text-4xl font-black text-slate-900 dark:text-white">
                {product.price.toLocaleString()} <span className="text-lg text-[#D4A017] font-bold">EGP</span>
              </span>
              {product.oldPrice && (
                <span className="text-lg text-slate-400 dark:text-gray-500 line-through">
                  {product.oldPrice.toLocaleString()} EGP
                </span>
              )}
              <span className="text-xs text-slate-500 dark:text-gray-400 ml-auto font-medium">Includes 14% Egyptian VAT</span>
            </div>

            {/* Vehicle Fitment Notice */}
            <div className="rounded-2xl border border-[#D4A017]/30 bg-amber-500/5 dark:bg-[#1B1B1B]/90 p-4">
              {selectedVehicle ? (
                ...
                <div className="flex items-center justify-between gap-3">
                  <div className="flex items-center gap-2.5">
                    ...
                    <span className="text-xs font-bold text-slate-900 dark:text-white">Select your vehicle to confirm fitment</span>
                  </div>
                  <button
                    onClick={() => setIsVehicleModalOpen(true)}
                    className="px-3 py-1.5 rounded-xl bg-[#8B3A2E] text-xs font-bold text-white hover:bg-[#a34436] transition shadow-sm"
                  >
                    Check Fitment
                  </button>
                </div>
            ...
            <div className="space-y-4 pt-4 border-t border-slate-200 dark:border-[#2D2D2D]">
              <div className="flex items-center gap-4">
                <span className="text-xs font-bold text-slate-500 dark:text-gray-400 uppercase">Quantity</span>
                <div className="flex items-center rounded-xl border border-slate-200 dark:border-[#2D2D2D] bg-slate-100 dark:bg-[#111111]">
                  <button
                    onClick={() => setQuantity((q) => Math.max(1, q - 1))}
                    className="p-2.5 text-slate-600 dark:text-gray-300 hover:text-slate-900 dark:hover:text-white transition"
                  >
                    <Minus className="h-4 w-4" />
                  </button>
                  <span className="px-4 py-2 text-sm font-bold text-slate-900 dark:text-white">{quantity}</span>
                  <button
            ...
            <div className="grid grid-cols-3 gap-3 pt-4 border-t border-slate-200 dark:border-[#2D2D2D] text-center text-[11px] font-semibold text-slate-600 dark:text-gray-400">
              <div className="flex flex-col items-center gap-1.5 p-2 rounded-xl bg-white dark:bg-[#1B1B1B] border border-slate-200 dark:border-transparent shadow-xs">
        ...
        {bundleItems.length > 0 && (
          <div className="mt-16 rounded-3xl border border-[#D4A017]/30 bg-white dark:bg-[#1B1B1B]/90 p-6 sm:p-8 backdrop-blur-2xl shadow-lg dark:shadow-2xl">
            ...
            <div className="flex flex-col md:flex-row items-center gap-6 justify-between">
              <div className="flex flex-wrap items-center gap-4">
                {/* Main item */}
                <div className="flex items-center gap-3 rounded-2xl border border-slate-200 dark:border-[#2D2D2D] bg-slate-50 dark:bg-[#111111] p-3 w-64">
                  <div className="relative h-14 w-14 shrink-0 bg-white dark:bg-[#1B1B1B] rounded-xl overflow-hidden">
              ...
              {/* Bundle Action */}
              <div className="border-t md:border-t-0 md:border-l border-slate-200 dark:border-[#2D2D2D] pt-4 md:pt-0 md:pl-6 text-center md:text-right shrink-0">
                <div className="text-xs text-slate-500 dark:text-gray-400">Total Bundle Price:</div>
                <div className="text-2xl font-black text-slate-900 dark:text-white">{bundleTotalPrice.toLocaleString()} <span className="text-sm text-[#D4A017]">EGP</span></div>
                <button
        ...
        <div className="mt-16 rounded-3xl border border-slate-200 dark:border-[#2D2D2D] bg-white dark:bg-[#1B1B1B] p-6 sm:p-8 backdrop-blur-xl shadow-lg dark:shadow-2xl">
          
          {/* Tab Navigation */}
          <div className="flex items-center gap-2 border-b border-slate-200 dark:border-[#2D2D2D] pb-4 overflow-x-auto">
            <button
              onClick={() => setActiveTab("specs")}
              className={`px-5 py-2.5 rounded-xl text-xs font-bold transition shrink-0 ${
                activeTab === "specs" ? "bg-[#8B3A2E] text-white shadow-md" : "text-slate-600 dark:text-gray-400 hover:text-slate-900 dark:hover:text-white"
              }`}
            >
            ...
            <button
              ...
              className={`px-5 py-2.5 rounded-xl text-xs font-bold transition shrink-0 ${
                activeTab === "compatibility" ? "bg-[#8B3A2E] text-white shadow-md" : "text-slate-600 dark:text-gray-400 hover:text-slate-900 dark:hover:text-white"
              }`}
            >
            ...
            <button
              ...
              className={`px-5 py-2.5 rounded-xl text-xs font-bold transition shrink-0 ${
                activeTab === "description" ? "bg-[#8B3A2E] text-white shadow-md" : "text-slate-600 dark:text-gray-400 hover:text-slate-900 dark:hover:text-white"
              }`}
            >
          ...
          <div className="mt-6">
            ...
            {activeTab === "specs" && (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {product.specifications.map((spec, idx) => (
                  <div key={idx} className="flex justify-between items-center p-3 rounded-xl bg-slate-50 dark:bg-[#111111] border border-slate-200 dark:border-[#2D2D2D] text-xs">
                    <span className="font-semibold text-slate-500 dark:text-gray-400">{spec.label}</span>
              ...
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  {product.compatibility?.map((comp, idx) => (
                    <div key={idx} className="flex items-center gap-3 p-3 rounded-xl bg-slate-50 dark:bg-[#111111] border border-slate-200 dark:border-[#2D2D2D]">
                      <CheckCircle className="h-4 w-4 text-emerald-600 dark:text-emerald-400 shrink-0" />
                      <div className="text-xs">
                        <div className="font-bold text-slate-900 dark:text-white">{comp.make} {comp.model} ({comp.yearStart}–{comp.yearEnd})</div>
                        {comp.engine && <div className="text-slate-500 dark:text-gray-400 text-[11px]">Engine: {comp.engine}</div>}
                      </div>
                    </div>
                  ))}
                </div>
            ...
            {activeTab === "description" && (
              <div className="space-y-6 text-sm text-slate-700 dark:text-gray-300 leading-relaxed">
                <p>{product.description}</p>

                {product.features && product.features.length > 0 && (
                  <div>
                    <h4 className="font-bold text-slate-900 dark:text-white text-base mb-3">Key Features & Benefits</h4>
        ...
        {relatedProducts.length > 0 && (
          <div className="mt-16">
            <h2 className="text-2xl font-black text-slate-900 dark:text-white mb-6">Related Automotive Products</h2>
