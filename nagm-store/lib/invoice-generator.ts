...
export async function generateInvoicePdf(order: Order): Promise<Uint8Array> {
  ...
  if (isCancelled) {
    ...
    orderStatusText = "Product Received";
    deliveryStatusText = "Delivered (Product Received)";
    totalLabel = "Grand Total Paid:";
  } else {
    ...
    if (isCod) {
      ...
      paymentStatusText = isPaid ? "Paid" : "Payment Pending";
      orderStatusText = isPaid ? "Product on the way to delivery" : "Pending Payment";
  ...
  page.drawText("Road 90, New Cairo, Cairo, Egypt | support@negmstore.com", {
    x: 108,
    y: height - 86,
    size: 8,
    font: fontRegular,
  ...
  page.drawText("Commercial Reg: EG-492-1082 | Tax ID: 928-441-01", {
    x: 108,
    y: height - 97,
    size: 7.5,
    font: fontRegular,
  ...
  const stateText = isDelivered ? "Status: COMPLETED" : "Status: IN PROGRESS";
  const stateWidth = fontBold.widthOfTextAtSize(stateText, 8);
  page.drawText(stateText, {
    x: width - 40 - stateWidth,
    y: height - 97,
    size: 8,
    font: fontBold,
    color: isDelivered ? colorGreen : colorGold,
  });
  ...
  const streetPart = address ? `${address.street || ""}, Bldg ${address.building || ""}` : "New Cairo";
  const cityPart = address ? `${address.city || ""}, ${address.governorate || "Cairo"}` : "Cairo, Egypt";
  ...
  // Right Column: Order Tracking & Status
  const rightColX = 330;
  page.drawText("DELIVERY & TRACKING:", {
    x: rightColX,
    y: infoStartY,
    size: 8.5,
    font: fontBold,
  ...
  const trkCode = order.trackingNumber || "To be assigned";
  page.drawText(`Tracking Code: ${trkCode}`, {
    x: rightColX,
    y: infoStartY - 50,
    size: 8,
    font: fontBold,
  ...
  // Limit items display safely if cart is large, ensuring strictly 1 page
  ...
  for (let i = 0; i < displayItems.length; i++) {
    ...
    // Product Name (truncated to avoid overflow)
    const rawName = item.product?.name || "Genuine Auto Part";
    const truncatedName = rawName.length > 44 ? `${rawName.slice(0, 42)}...` : rawName;
    ...
    // Unit Price
    const unitPriceText = `${(item.product?.price || 0).toLocaleString()} EGP`;
    ...
    // Line Total
    const lineTotalText = `${((item.product?.price || 0) * item.quantity).toLocaleString()} EGP`;
  ...
  const tableBottomY = currentY - displayItems.length * rowHeight;
  page.drawLine({
  ...
  // ==========================================
  const summaryTopY = tableBottomY - 20;
  ...
  const paymentNote = isPaid
    ? "Payment has been processed and confirmed in full."
    : isCod
  ...
  // Guarantee badge
  page.drawText("100% Genuine Auto Parts Guarantee", {
    x: 50,
    y: summaryTopY - 78,
    size: 7.5,
    font: fontBold,
  ...
  page.drawText("All items verified with official distributor batch codes.", {
    x: 50,
    y: summaryTopY - 90,
    size: 7,
    font: fontRegular,
  ...
  page.drawText(`${subtotal.toLocaleString()} EGP`, {
    x: sumValX,
    y: summaryTopY - 12,
    size: 8.5,
    font: fontBold,
  ...
  if (discount > 0) {
    ...
    page.drawText(`-${discount.toLocaleString()} EGP`, {
      x: sumValX,
      y: summaryTopY - 26,
      size: 8.5,
      font: fontBold,
  ...
  // Shipping
  const shipY = discount > 0 ? summaryTopY - 40 : summaryTopY - 26;
  ...
  page.drawText(shipping === 0 ? "FREE" : `${shipping.toLocaleString()} EGP`, {
    x: sumValX,
    y: shipY,
    size: 8.5,
    font: fontBold,
  ...
  // VAT (14%)
  const vatY = discount > 0 ? summaryTopY - 54 : summaryTopY - 40;
  ...
  page.drawText(`${vat.toLocaleString()} EGP`, {
    x: sumValX,
    y: vatY,
    size: 8.5,
    font: fontBold,
  ...
  // Divider above total
  const totalDivY = discount > 0 ? summaryTopY - 65 : summaryTopY - 52;
  page.drawLine({
    start: { x: sumRightX, y: totalDivY },
    end: { x: width - 40, y: totalDivY },
    thickness: 1,
    color: colorBorder,
  });

  // Total Box
  const totalBoxY = totalDivY - 28;
  page.drawRectangle({
    x: sumRightX - 4,
    y: totalBoxY,
    width: width - 40 - (sumRightX - 4),
    height: 24,
  ...
  page.drawText(`${grandTotal.toLocaleString()} EGP`, {
    x: sumValX - 10,
    y: totalBoxY + 7,
    size: 11,
    font: fontBold,
  ...
  // ==========================================
  // 5. DEVELOPER ATTRIBUTION (BOTTOM OF SINGLE A4 PAGE)
  // ==========================================
  const bottomDividerY = 100;
  page.drawLine({
  ...
  // Clickable "Contact Developer" Button Box
  const btnX = 320;
  const btnY = 64;
  const btnWidth = 235;
  const btnHeight = 22;
  ...
  page.drawLine({
    start: { x: envX, y: envY + 9 },
    end: { x: envX + 6, y: envY + 4 },
    thickness: 0.8,
    color: colorPrimary,
  });
  page.drawLine({
    start: { x: envX + 12, y: envY + 9 },
    end: { x: envX + 6, y: envY + 4 },
    thickness: 0.8,
    color: colorPrimary,
  });

  const btnText = `Contact Developer ->   ${DEVELOPER_EMAIL}`;
  ...
  // Clickable URI Annotation for [ Contact Developer ]
  try {
  ...
  // Return PDF bytes (discarded after sending)
  return pdfDoc.save();
}
